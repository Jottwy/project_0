...
fn level0_has_hallways() {
    ...
    let hallway_count = chunks
        .iter()
        .filter(|(_, c)| {
            c.template_id == TEMPLATE_HALLWAY_STRAIGHT || c.template_id == TEMPLATE_HALLWAY_CORNER
        })
        .count();
...
fn edges_connect(layout: &ChunkLayoutV1, from: u8, to: u8) -> bool {
    ...
    let targets: HashSet<(usize, usize)> = boundary_opening_cells(layout, to).into_iter().collect();
    if targets.is_empty() {
        return false;
    }
...
fn seed_7778_has_reachable_vertical_showcase() {
    ...
    let (_, sc) = showcase.expect("seed 7778 has a reachable vertical showcase in radius 3..=10");
    assert!(
