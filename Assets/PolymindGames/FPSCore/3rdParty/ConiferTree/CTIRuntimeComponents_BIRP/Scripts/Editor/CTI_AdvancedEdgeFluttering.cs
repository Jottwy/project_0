...
namespace CTI
{
    public sealed class CTI_AdvancedEdgeFluttering : MaterialPropertyDrawer
    {
        public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
        {
            ...
            if (material.GetFloat("_EnableAdvancedEdgeBending") != 0)
            {
                ...
                Rect pos_1 = new(position.position.x, position.position.y, position.width, halfHeight);
                Rect pos_2 = new(position.position.x, position.position.y + halfHeight + 2, position.width, halfHeight);
