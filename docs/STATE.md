...
- **SIN VER EN PLAY**: linterna (ADR-133) y venda (`bb9e3cc1`). ADR-135 R1 (Steam) YA verificado en Play, ver Riesgos abiertos.
...
- **Steam (ADR-135 enm. 2) VERIFICADO el 09-09 con redes y cuentas Steam distintas: el criterio físico ya se cumple.** Sigue sin probar
  el relay propio (`DefaultRelayAddress` vacío, ADR-117 sin VPS) — queda como vía alternativa, ya NO bloquea Alpha 1.
...
- **Verticalidad jugable** (cruzar de capa por escalera o hueco): diferida a post-Alpha 1 con ADR propio, nota anclada en `DECISIONS.md`;
  `require_walkable_above`/`_below` se generan y ningún llamador los consume.
