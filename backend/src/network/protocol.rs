...
mod tests {
  ...
  ")
    ...
    fn pose_wire_round_trips_thin_and_full() {
        ...
        assert_eq!(
            (hit_seq, fire_seq, buttons, melee_seq, vocal_seq),
            (7, 9, 0b11, 4, 6)
        );
        ...
        assert_eq!(
            (equipment, held_item, species),
            ([101, 202, 303, 404], 12345, 2)
        );
    }
    ...
    fn quantization_saturates_and_yaw_wraps() {
        ...
        assert_eq!(
            PoseWire::quantize_pos([1000.0, 0.0, -1000.0], origin),
            [i16::MAX, 0, i16::MIN]
        );
        assert_eq!(
            PoseWire::quantize_pos([0.004, 0.006, -0.006], origin),
            [0, 1, -1]
        );
        assert_eq!(PoseWire::quantize_yaw(360.0), 0);
        assert_eq!(PoseWire::quantize_yaw(-90.0), PoseWire::quantize_yaw(270.0));
        assert!(
            (PoseWire::dequantize_yaw(PoseWire::quantize_yaw(123.456)) - 123.456).abs() < 0.006
        );
    }
