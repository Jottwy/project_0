...
pub fn is_showcase_chunk_pos(pos: ChunkPos) -> bool {
...
impl VolumetricGridV0 {
    ...
    //// Neighbour occupancy in a direction. Out-of-grid is `Solid` so the volume
    /// remains fully enclosed: perimeter walls, bottom floor and top roof.
    pub fn neighbour(&self, x: u8, y: u8, z: u8, dir: FaceDir) -> CellOccupancy {
        let (dx, dy, dz) = dir.delta();
        self.cell_or_solid(x as i32 + dx, y as i32 + dy, z as i32 + dz)
    }
    ...
    /// Out-of-grid cells are treated as solid so the volume stays enclosed.
    #[inline]
    fn cell_or_solid(&self, x: i32, y: i32, z: i32) -> CellOccupancy {
        if self.in_bounds(x, y, z) {
            self.cell(x as u8, y as u8, z as u8)
        } else {
            CellOccupancy::Solid
        }
    }
    ...
    /// Iteration order is fixed (`y`, then `z`, then `x`) so the output is stable
    /// for a given grid.
    pub fn generate_faces(&self) -> Vec<Face> {
        let total_cells = self.nx as usize * self.ny as usize * self.nz as usize;
        let mut faces = Vec::with_capacity(total_cells * 4);
        ...
        for y in 0..self.ny {
            for z in 0..self.nz {
                for x in 0..self.nx {
                    ...
                    let north = self.cell_or_solid(xi, yi, zi + 1);
                    let south = self.cell_or_solid(xi, yi, zi - 1);
                    let east = self.cell_or_solid(xi + 1, yi, zi);
                    let west = self.cell_or_solid(xi - 1, yi, zi);
                    let below = self.cell_or_solid(xi, yi - 1, zi);
                    let above = self.cell_or_solid(xi, yi + 1, zi);

                    let wall_kind = if cell.is_vertical_void() {
                        FaceKind::ShaftWall
                    } else {
                        FaceKind::Wall
                    };

                    if north.blocks_horizontally() {
                        faces.push(Face {
                            x,
                            y,
                            z,
                    ...
                    // Floor is omitted only when a real vertical void continues downward.
                    ...
                    // Ceiling is omitted only when a real vertical void continues upward.
                    ...
                    // Rim + railing around true lateral vertical openings.
                    if cell.is_walkable() {
                        if north.is_vertical_void() {
                            faces.push(Face {
                                x,
                                y,
                                z,
                    ...
                    // Structural support marker for support-core cells.
...
fn push_macro_access(
    ...
    access.push(VerticalAccessNode {
        access_id: access_id(world_seed, chunk.pos, 64 + index),
        access_type,
        from_layer,
        to_layer,
...
fn apply_vertical_macrostructure_v30e(
    world_seed: u64,
    chunk: &Chunk,
    grid: &mut VolumetricGridV0,
    access: &mut Vec<VerticalAccessNode>,
    macrostructure: VerticalMacrostructureV0,
) {
    match macrostructure {
        VerticalMacrostructureV0::Normal => {}

        VerticalMacrostructureV0::TallPillarHall => {
            let pillars = [
                (2u8, 2u8),
                (7u8, 2u8),
                (2u8, 7u8),
            ...
            for (x, z) in pillars {
                if grid.cell(x, 1, z).is_open() {
                    grid.set(x, 0, z, CellOccupancy::SupportCore);
                    grid.set(x, 1, z, CellOccupancy::SupportCore);
                    grid.set(x, 2, z, CellOccupancy::SupportCore);
                }
            }
        ...
        VerticalMacrostructureV0::VoidGrateRoom => {
            for z in 3u8..=6u8 {
                for x in 3u8..=6u8 {
                    if (x + z) % 2 == 0 && grid.cell(x, 1, z).is_walkable() {
                        grid.set(x, 0, z, CellOccupancy::Shaft);
                        grid.set(x, 1, z, CellOccupancy::AtriumVoid);
                        grid.set(x, 2, z, CellOccupancy::CeilingVoid);
                    }
                }
            }
        ...
        VerticalMacrostructureV0::StackedAtrium => {
            for z in 3u8..=6u8 {
                for x in 3u8..=6u8 {
                    if grid.cell(x, 1, z).is_walkable() {
                        grid.set(x, 0, z, CellOccupancy::AtriumVoid);
                        grid.set(x, 1, z, CellOccupancy::AtriumVoid);
                        grid.set(x, 2, z, CellOccupancy::AtriumVoid);
                    }
                }
            }
        ...
        VerticalMacrostructureV0::ServiceShaftDeep => {
            for z in 4u8..=5u8 {
                for x in 4u8..=5u8 {
                    if grid.cell(x, 1, z).is_open() {
                        grid.set(x, 0, z, CellOccupancy::Shaft);
                        grid.set(x, 1, z, CellOccupancy::Shaft);
                        grid.set(x, 2, z, CellOccupancy::Shaft);
                    }
                }
            }
        ...
        VerticalMacrostructureV0::EndlessColumnVoid => {
            for z in 4u8..=5u8 {
                for x in 4u8..=5u8 {
                    if grid.cell(x, 1, z).is_open() {
                        grid.set(x, 0, z, CellOccupancy::Shaft);
            ...
            let supports = [(3u8, 3u8), (6u8, 3u8), (3u8, 6u8), (6u8, 6u8)];
            for (x, z) in supports {
                if grid.cell(x, 1, z).is_open() {
                    grid.set(x, 0, z, CellOccupancy::SupportCore);
                    grid.set(x, 1, z, CellOccupancy::SupportCore);
...
fn protect_spawn_core_v30e(chunk: &Chunk, grid: &mut VolumetricGridV0) {
    if chunk.pos != (0, 0) {
        return;
    }

    // Keep the spawn core and a small movement cross clear on the main band.
    // This lets the player spawn inside the adapted chunk without being trapped
    // by a shaft, atrium, pillar, anomaly, or decorative void.
    for z in 3u8..=6u8 {
        for x in 3u8..=6u8 {
            grid.set(x, 1, z, CellOccupancy::Room);
        }
    }

    // Keep upper/lower architecture readable but non-blocking around spawn.
    // No physical vertical opening directly under/above the spawn core.
    for z in 4u8..=5u8 {
        for x in 4u8..=5u8 {
            grid.set(x, 0, z, CellOccupancy::ServiceSpace);
            grid.set(x, 1, z, CellOccupancy::Room);
            grid.set(x, 2, z, CellOccupancy::CeilingVoid);
        }
    }

    // Simple cross-shaped walkable exit from the spawn center.
    for i in 0u8..LAYOUT_GRID_SIZE {
        grid.set(i, 1, 5, CellOccupancy::Corridor);
        grid.set(5, 1, i, CellOccupancy::Corridor);
    }
}
...
pub fn build_level0_column(world_seed: u64, chunk: &Chunk) -> VolumetricColumn {
    ...
    let macrostructure = vertical_macrostructure_for(world_seed, chunk);

    if macrostructure != VerticalMacrostructureV0::Normal {
        apply_vertical_macrostructure_v30e(
            world_seed,
            chunk,
            &mut grid,
            &mut vertical_access,
        ...
        protect_spawn_core_v30e(chunk, &mut grid);
    }
...
mod tests {
    ...
    fn assert_unique_access_ids(view: &VolumetricGridViewV0) {
        for i in 0..view.vertical_access.len() {
            for j in (i + 1)..view.vertical_access.len() {
                assert_ne!(
                    view.vertical_access[i].access_id, view.vertical_access[j].access_id,
                    "duplicate vertical_access id detected: {}",
                    view.vertical_access[i].access_id
                );
            }
        }
    }
    ...
    #[test]
    fn level0_column_vertical_access_ids_are_unique() {
        let seed = 42u64;
        let chunks = crate::world::generator::generate_initial_structure_chunks(seed);
    ...
    fn physical_vertical_openings_only_exist_when_explicitly_authored() {
        ...
        assert_unique_access_ids(&normal_view);
        assert!(
            normal_view
                .vertical_access
                .iter()
