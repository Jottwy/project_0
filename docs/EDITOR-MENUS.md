...
| `Backrooms/Screwdriver/Aplicar modelo Meshy` | `Editor/BackroomsScrewdriverModelApplier.cs` | calcado del bote: hornea el modelo de `Assets/MeshyImports/Screwdriver/` (gitignored) a `Assets/Art/Items/Screwdriver/` (malla canónica de 24 cm, punta a +Y detectada por grosor, texturas a 1024, material URP), lo cuelga de `Hand.R` en `BR_Wieldable_Screwdriver`, apaga el nodo `Axe`, **encadena el pickup** y deja dos renders del modelo (fondo negro/blanco) en el scratchpad para el icono. Reejecutable |
| `Backrooms/Screwdriver/Crear el destornillador del suelo` | `Editor/BackroomsScrewdriverPickupCreator.cs` | clona `STP_Pickup_HuntingAxe` a `Assets/Prefabs/Items/BR_Pickup_Screwdriver.prefab`, fuera LODs del hacha, malla/material horneados, caja del collider de la malla, `_item` al id, masa = peso, `_pickup` de la definición y refresco de la `SaveableDatabase`. También es lo que cuelga `ProxyHeldItemHook` de la mano del avatar remoto |
| `Backrooms/Screwdriver/Asignar icono` | `Editor/BackroomsScrewdriverModelApplier.cs` | importa `Assets/Art/Items/BR_Screwdriver_Icon.png` como Sprite FullRect y lo asigna. El PNG sale de los dos renders del modelo: alfa por diferencia, recorte, 25°, 94 % del lienzo — el mismo tratamiento que el bote y el agua |
