...
namespace BackroomsSurvival.Gameplay.Building
{
    ...
    public sealed class GridWallBuildingPiece : BuildingPiece, ISaveableComponent
    {
        ...
        [Tooltip("Columns of the infill grid across the 5 m span. 5 columns against 4 rows gives an " +
                 "exactly square 1 × 1 m cell, which is the only subdivision that leaves the drywall " +
                 "sheet (1.900 × 1.884 native) undistorted.")]
        private int _gridColumns = 5;

        [SerializeField, Range(1, 8)]
        [Tooltip("Rows of the infill grid up the 4 m height. See _gridColumns for why 5 × 4. " +
                 "Retuning either value does NOT resize the panel prefabs: a panel's scale is baked " +
                 "into its asset because the replicated copy is spawned from def_id + pose with no " +
                 "scale on the wire. Change these and the panel prefabs must be regenerated by hand.")]
        ...
        /// <summary>
        /// Columns of the infill grid a <see cref="GridPanelBuildingPiece"/> clads into. Read off the
        /// frame rather than baked into the snapper so the subdivision stays tunable per prefab.
        /// </summary>
        public int GridColumns => _gridColumns;

        /// <summary>Rows of the infill grid. See <see cref="GridColumns"/>.</summary>
...
#if UNITY_EDITOR
        /// <summary>
        /// Draws the infill grid on both faces so the subdivision can be judged against the frame's
        /// actual bars in the scene view — the Steel Frame Grid is an authored mesh, so whether a
        /// given columns × rows lines up with it is a thing you look at, not a thing you compute.
        /// </summary>
        private void OnDrawGizmosSelected()
        {
            var cell = GridPanelSnap.CellSize(_gridColumns, _gridRows);
            float halfLength = GridPanelSnap.SlotLength * 0.5f;
            ...
            {
                float z = (face == 0 ? 1f : -1f) * GridPanelSnap.SlotHalfThickness;

                for (int column = 0; column <= _gridColumns; column++)
                {
                    float x = -halfLength + column * cell.x;
                    Gizmos.DrawLine(new Vector3(x, 0f, z), new Vector3(x, GridPanelSnap.SlotHeight, z));
                }

                for (int row = 0; row <= _gridRows; row++)
                {
                    float y = row * cell.y;
                    Gizmos.DrawLine(new Vector3(-halfLength, y, z), new Vector3(halfLength, y, z));
                }
            }
